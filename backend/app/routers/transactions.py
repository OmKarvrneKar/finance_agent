...
async def upload_statement(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"AI API is not configured properly: {str(e)}"
        )
    except RuntimeError as e:
        logger.error(f"AI API failure: {str(e)}")
        raise HTTPException(
    ...
    # 3. Save to database
    try:
        db_txs, duplicates = crud.create_transactions(db, categorized_transactions, user_id=current_user.id)
    except Exception as e:
