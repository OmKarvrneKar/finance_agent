...
def test_budget_status_with_decimal_cap(db):
    bg = BudgetGoal(category="Food", monthly_cap=Decimal('100.00'))
    db.add(bg)
    t = Transaction(date=date.today(), description="Lunch", amount=Decimal('45.50'), transaction_type="debit", category="Food")
    db.add(t)
    db.commit()
...
def test_budget_status_over_decimal(db):
    bg = BudgetGoal(category="Shopping", monthly_cap=Decimal('200.00'))
    db.add(bg)
    t = Transaction(date=date.today(), description="Gadget", amount=Decimal('250.00'), transaction_type="debit", category="Shopping")
    db.add(t)
    db.commit()
...
def test_forecasting_with_decimal_amounts(db):
    t1 = Transaction(date=date(2026, 7, 1), description="Food", amount=Decimal('15.50'), transaction_type="debit", category="Food")
    t2 = Transaction(date=date(2026, 7, 3), description="Food", amount=Decimal('22.75'), transaction_type="debit", category="Food")
    t3 = Transaction(date=date(2026, 7, 5), description="Food", amount=Decimal('31.25'), transaction_type="debit", category="Food")
    db.add_all([t1, t2, t3])
    db.commit()
...
def test_anomaly_detection_with_decimal(db):
    t1 = Transaction(date=date(2026, 1, 1), description="Netflix", amount=Decimal('10.00'), transaction_type="debit", category="Ent", is_recurring=True)
    t2 = Transaction(date=date(2026, 2, 1), description="Netflix", amount=Decimal('10.00'), transaction_type="debit", category="Ent", is_recurring=True)
    t3 = Transaction(date=date(2026, 3, 1), description="Netflix", amount=Decimal('15.00'), transaction_type="debit", category="Ent", is_recurring=True)
    db.add_all([t1, t2, t3])
    db.commit()
...
def test_savings_goal_with_decimal(db):
    sg = SavingsGoal(name="Vacation", target_amount=Decimal('3000.00'))
    db.add(sg)
    db.commit()
...
def test_simulate_what_if_decimal(db):
    h1 = Transaction(date=date(2025, 1, 10), description="old", amount=Decimal('100.00'), transaction_type="debit", category="Food")
    h2 = Transaction(date=date(2025, 2, 10), description="old", amount=Decimal('100.00'), transaction_type="debit", category="Food")
    h3 = Transaction(date=date(2025, 3, 10), description="old", amount=Decimal('100.00'), transaction_type="debit", category="Food")
    db.add_all([h1, h2, h3])
    db.commit()
